...
func Router() {
	...
	secure := mainRouter.PathPrefix("/v1").Subrouter()
	secure.Use(middleware.ParameterMiddleware, middleware.MiddlewareJWT)
	...
	secure.HandleFunc("/category/{id}", controller.UpdateCategory).Methods("PUT")
	...
	secure.HandleFunc("/product/{id}", controller.UpdateProduct).Methods("PUT")
	...
	secure.HandleFunc("/modifier/{id}", controller.UpdateModifier).Methods("PUT")
	...
	secure.HandleFunc("/location/{id}", controller.UpdateLocation).Methods("PUT")
